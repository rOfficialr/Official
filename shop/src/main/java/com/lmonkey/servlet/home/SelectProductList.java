...
//        通过父分类ID或者子分类ID得到该分类下所有商品
        ...
        if (fid!= null){
            ...
            list = lmonkey_productService.selectByFid(Integer.parseInt(fid));
        }
        if(cid!=null){
            id = Integer.parseInt(cid);
            list = lmonkey_productService.selectByCid(Integer.parseInt(cid));
        }
        req.setAttribute("list",list);
        String cate_name = lmonkey_categoryService.selectByID(id).getCate_name();
        req.setAttribute("title",cate_name);
